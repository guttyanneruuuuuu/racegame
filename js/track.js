...
const Track = {
  ...
  _buildJumpPads() {
    ...
    for (const t of positions) {
      ...
      steam.position.set(px, 3.0, pz);
      this.group.add(steam);

      this.jumpPads.push({
        mesh: ringMesh, glow: inner, steam, x: px, z: pz, dirX, dirZ,
        radius: 4.0,
        _phase: Math.random() * Math.PI * 2,
        _lastTrigger: new Map(),
      });
  ...
  _buildLavaPools() {
    ...
    for (const t of positions) {
      ...
      this.lavaPools.push({
        mesh: m, glow, x: px, z: pz, radius: 2.6,
        _phase: Math.random() * Math.PI * 2,
        _lastTrigger: new Map(),
      });
    }
  ...
  _buildBoulders() {
    ...
    for (const s of seeds) {
      ...
      m.position.set(cur.x, 1.6, cur.z);
  ...
  update(dt, now) {
    ...
    for (const p of this.jumpPads) {
      ...
      if (p.steam) {
        ...
        p.steam.scale.y = sy;
        p.steam.position.y = 3.0 + (sy - 1) * 1.6;
